...
function Badge({ ok, label }) {
  return (
    <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border ${
      ok ? "bg-emerald-900/40 text-emerald-400 border-emerald-700/40"
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-slate-400 text-sm mt-1">Live health check for all services</p>
      </div>
      ...
      {health && (
        <>
          <div className={`mb-8 flex items-center gap-3 p-4 rounded-xl border ${
            ...
            <div className={`w-3 h-3 rounded-full ${health.status === "ok" ? "bg-emerald-400 animate-pulse" : "bg-red-400"}`} />
            <span className={`font-semibold ${health.status === "ok" ? "text-emerald-300" : "text-red-300"}`}>
              {health.status === "ok" ? "All systems operational" : "Service degraded"}
            </span>
            <span className="ml-auto text-xs text-slate-500">{health.timestamp}</span>
          </div>

          <div className="grid sm:grid-cols-2 gap-4 mb-6">
            {[
              ["Express API", health.services.express === "ok", health.services.express, "Node.js REST server"],
              ["MongoDB", health.services.mongodb === "connected", health.services.mongodb, "Mongoose ODM"],
              ...
              <div key={name} className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                ...
                <p className="text-2xl font-bold text-white">{ok ? "Online" : "Offline"}</p>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-slate-800">
              <h2 className="font-medium text-slate-300 text-sm">Environment</h2>
            </div>
            <div className="divide-y divide-slate-800">
              {[
                ...
                <div key={k} className="flex items-center px-5 py-3 hover:bg-slate-800/30">
          ...
          <div className="mt-4 bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-slate-800">
              <h2 className="font-medium text-slate-300 text-sm">API Endpoints</h2>
            </div>
            <div className="divide-y divide-slate-800">
              {[
                ...
                <div key={path} className="flex items-center px-5 py-3">
                  <span className={`text-xs font-mono font-bold w-16 ${
                    ...
                    method === "POST" ? "text-blue-400" : "text-red-400"
                  }`}>{method}</span>
